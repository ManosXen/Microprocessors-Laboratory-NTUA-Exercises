...
int main(void) {
    ...
    PCA9555_0_write(REG_CONFIGURATION_1, 0xFE); //Set EXT_PORT0 as output
    ...
    while(1)
    {
        ...
        if(input & 0x10) output|=0x01;

        if(input & 0x20) output|=0x02;

        if(input & 0x40) output|=0x04;

        if(input & 0x80) output|=0x08;
